...
class CliffState(SpecialState):
    """
    Class representing cliff states in the gridworld. Cliff states are terminal states, meaning that once the agent transitions to a cliff state, it remains in that state and receives the same reward for every subsequent transition.
    """
    def __init__(self, location, name='cliff', color='red', reward_to=-100.0, **kwargs):

        super().__init__(location=location, name=name, color=color, reward_to=reward_to, terminal=False, **kwargs)
...
class GridWorld(object):
            ...
            for a in self.actions:
                ...
                if (s in self.special_states.keys()) or (s_new in self.special_states.keys()):
                    if s in self.special_states.keys():
                        ...
                        elif self.special_states[s].name == 'cliff': # cliff states move the agent to the initial state
                            if self.special_states[s].reward_from is None:
                                reward_from = self.default_reward
                            else:
                                reward_from = self.special_states[s].reward_from
                            p[(s,a)] = {(self.initial_state, reward_from) : 1.0}
                        else:
                    ...
                    if s_new in self.special_states.keys():
                        ...
                        if self.special_states[s_new].name == 'hard_wall': # hard wall states
                            ...
                            p[(s,a)] = {(s_new, reward_to) : 1.0}
                        elif self.special_states[s_new].name == 'cliff': # cliff states move the agent to the initial state
                            p[(s,a)] = {(self.initial_state, reward_to) : 1.0}
                        else:
                            p[(s,a)] = {(s_new, self.default_reward) : 1.0}
                else:
                    p[(s,a)] = {(s_new, self.default_reward) : 1.0}
    ...
    def plot_v(self, ax=None, v=None, title="State value function", print_states=False, **kwargs):
        ...
        for s in self.states:
            if s in self.special_states.keys():
                if self.special_states[s].name == 'hard_wall' or self.special_states[s].name == 'cliff': # do not print value for hard wall states and cliff states
                    pass
                else:
    ...
    def plot_policy(self, ax=None, policy=None, title="Policy", print_states=False, **kwargs):
        ...
        # Print policy
        for s in self.states:
            do_not_print_policy = False
            if s in self.special_states.keys():
                if self.special_states[s].name == 'hard_wall' or self.special_states[s].name == 'cliff': # do not print policy for hard wall states and cliff states
                    do_not_print_policy = True
            if not do_not_print_policy:
...
class CliffWalking_Example(GridWorld):
    """
    Gridworld with a start state, a terminal state and cliff states. The start state is located at the bottom left corner 
    of the gridworld, the terminal state is located at the bottom right corner of the gridworld and the cliff states are located in between the start and terminal states. The cliff states give a large negative reward and are absorbing states, meaning that once the agent transitions to a cliff state, it remains in that state and receives the same reward for every subsequent transition.
    """
    def __init__(self, grid_size=(4,12), default_reward=-1.0, special_states=None):
        
        if special_states is None:
            special_states = ([StartState(location=36),
                               TerminalState(location=47, reward_to=-1.0)]
                               +
                               [CliffState(location=i) for i in range(37, 47)])
